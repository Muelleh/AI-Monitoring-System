...
def simulate_single(profile="standard", seed=None):
...
def simulate_team(n=10, names=None, seed=42):
    rng = np.random.default_rng(seed)
    rows = []
    default_names = ["Alpha-1","Alpha-2","Alpha-3","Bravo-1","Bravo-2",
                     "Bravo-3","Charlie-1","Charlie-2","Delta-1","Delta-2"]
    names = names or default_names
    for i in range(n):
        profile = rng.choice(list(PROFILES.keys()), p=[0.3,0.5,0.2])
        ...
        rows.append(row)
    cols = ["id","name","role","heart_rate","sleep_hours","training_load",
            "body_temp","recovery_indicator","date"]
